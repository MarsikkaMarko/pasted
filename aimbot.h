...
uint64_t GetEntityByID(int id)
{
	...
	return Interface->Read<uint64_t>( entityList + ( sizeof( std::uint64_t ) * id ) );
}

uint64_t DecryptEntityInfo(uint64_t entity)
{
	return ((Interface->Read<std::uint64_t>(entity + 0x50) ^ 0x59089C3C53641909) + 0x3E48F409BF6F5A50) ^ 0x155D2A1FBD952158;
}
...
inline BYTE entity_team(uint64_t entity)
{
	...
	return Interface->Read<BYTE>(teamInfo + 0x30);
}
...
inline uint64_t game_camera()
{
	...
	tmp = Interface->Read<uint64_t>(tmp + 0x220);
